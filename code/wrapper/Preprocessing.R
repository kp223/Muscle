...
pacman::p_load(RSpectra,qs,reshape2,dplyr,rTensor,Matrix,data.table,gtools,purrr,furrr)
options(scipen = 4)
...
if(Rank_HiC==0){
  ...
  Rank_HiC <- as.numeric(readLines(con="stdin", 1))
  cat(Rank_HiC, "\n")
  
  
  
  #check rank truncation first
  if(is.na(Rank_HiC)){Rank_HiC=30}
}    

saveRDS(Rank_HiC,paste0(dir_data,'/Rank.rds'))
...
svd_fitted=svd_res$u[,1:Rank_HiC]%*%diag(svd_res$d[1:Rank_HiC])%*%t(svd_res$v[,1:Rank_HiC])
rm(svd_res)
...
if(debias==TRUE){
  ...
  tmp_wide_list=split(tmp_wide,f=tmp_wide$chrom)
  rm(summarized_hic)
  rm(input_mat)
  gc()
  library(purrr)
  ...
  furrr::future_map(tmp_wide_list,function(x){  
    which_chr=x$chrom[1]
    tmp_long <- melt( x, id.vars = c("chrom","binA","binB"))
    rm(x)
    ...
    gc()
    qs::qsave(input_mat,paste0(dir_data,"/Matricized_HiCtensor_imputed_",which_chr,".qs"))
    qs::qsave(summarized_hic,paste0(dir_data,"/summarized_hic_",which_chr,".qs"))
    rm(input_mat)
    rm(summarized_hic)
    gc()
    #return(input_mat)  
...
if(debias!=TRUE){
  ...
  #qs::qsave(summarized_hic,'summarized_hic.qs')
  lapply(split(summarized_hic,f="chrom"),function(x)qs::qsave(x,paste0(dir_data,"/summarized_hic_",x$chrom[1],".qs")))
...
if(GNU==TRUE){
  if(!is.null(ssh))system(paste0('cd ',dir_functions,'; parallel -S ',ssh,' --jobs 4 --workdir . Rscript ::: tensor_generator.R ::: ', which.chrs,' ::: ',dir_data," ::: ",dir_functions," ::: ",chr_num," ::: ",sizefile," ::: ",binsize))
  if(is.null(ssh))system(paste0('cd ',dir_functions,'; parallel --jobs 4 --workdir . Rscript ::: tensor_generator.R ::: ', which.chrs,' ::: ',dir_data," ::: ",dir_functions," ::: ",chr_num," ::: ",sizefile," ::: ",binsize))
} 
if(GNU!=TRUE){
  for(chr in 1:chr_num){
    
    system(paste0('cd ',dir_functions,"; Rscript --vanilla tensor_generator.R ",chr,' ',' ',dir_data,' ',dir_functions,' ',chr_num,' ',sizefile,' ',binsize))
  }
